namespace DropWord.TgBot.Core.Handler.TaskProcessingHandler.Implementation;

public class BackgroundTaskHandler: IBackgroundTaskHandler
{
    private readonly Dictionary<string, Dictionary<long, (Task Task, CancellationTokenSource CancellationTokenSource)>> _taskDictionary;

    public BackgroundTaskHandler()
    {
        _taskDictionary = new Dictionary<string, Dictionary<long, (Task Task, CancellationTokenSource CancellationTokenSource)>>();
    }
    
    public Task StartProcessAsync<T>(long userId, string nameProcess, Func<T, CancellationToken, Task> func, T model) where T : class
    {
        var cancellationTokenSource = new CancellationTokenSource();
        var cancellationToken = cancellationTokenSource.Token;

        // Создаём и запускаем задачу асинхронной обработки
        var task = Task.Run(() => func(model, cancellationToken),
            cancellationToken);
        ...
        lock (_taskDictionary)
        {
            ...
            {
                _taskDictionary.Add(nameProcess, new Dictionary<long, (Task Task, CancellationTokenSource CancellationTokenSource)>());
            }
            _taskDictionary[nameProcess].Add(userId, (task, cancellationTokenSource));
        }
